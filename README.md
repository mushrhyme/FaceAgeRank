...
- 실시간 랭킹 시스템
- 구글 시트 연동을 통한 데이터 저장
- 음향 효과 (웰컴, 카운트다운, 촬영, 분석 중, 결과)
- 시각적 효과 (동안: 팡파레, 노안: 리플 및 화면 진동)
...
# 얼굴 나이 분석 모델 경로 (기본값: ./age_estimation.onnx)
...
- **분석 결과**: 얼굴 나이 및 실제 나이 비교
  - 동안일 때: 팡파레 효과 및 축하 음향
  - 노안일 때: 리플 효과, 화면 진동 효과 및 음향
- **랭킹보드**: 
...
시스템은 ONNX 형식의 얼굴 나이 분석 모델을 사용합니다. 기본 모델 경로는 `./age_estimation.onnx`입니다.

다른 경로의 모델을 사용하려면 환경 변수를 설정하세요:

```env
FACE_AGE_MODEL_PATH=./path/to/your/model.onnx
```
...
- **사번**: 직원 사번 (문자열로 처리)
- **이름**: 직원 이름
- **출생년도**: 8자리 숫자 형식 (예: `19970919`) - **필수**
- **성별**: 성별 정보

**중요**: 출생년도는 반드시 8자리 숫자 형식(`19970919`)이어야 합니다. 프로그램은 이 값을 오늘 날짜 기준으로 만 나이로 자동 계산합니다.
...
3. 엑셀에서 찾으면: 이름, 출생년도, 부서 정보 읽기
4. 출생년도(8자리 숫자)를 오늘 날짜 기준으로 **만 나이** 자동 계산
5. 사용자 정보를 화면에 표시 (환영 화면, 결과 화면 등)
6. 얼굴 촬영 후 이미지 자동 압축 (최대 너비 800px, JPEG 품질 0.7)
7. 압축된 이미지를 서버로 전송하여 AI 모델로 얼굴 나이 분석
8. 결과 화면에 실제 나이와 얼굴 나이 비교 표시
9. 동안일 경우 팡파레 효과, 노안일 경우 리플 효과 및 화면 진동 효과 표시
