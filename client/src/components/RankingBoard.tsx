...
export default function RankingBoard() {
  ...
  const getRankBadge = (rank: number) => {
    ...
    if (rank === 2) {
      // 2위는 20% 어두운 톤 (#39E5C2의 80% = 약 #2DB89A)
      return <span className={`${commonStyle}`} style={{ color: '#2DB89A' }}>2위</span>;
    }
    if (rank === 3) {
      return <span className={`${commonStyle} text-[#39E5C2]`}>3위</span>;
    }
    return <span className={`${commonStyle} text-gray-300`}>{rank}위</span>;
  };

  // 순위별 배경색 반환 (1위는 더 밝게)
  const getRowBackgroundColor = (rank: number) => {
    if (rank === 1) {
      return '#253030'; // 1위는 더 밝게
    }
    if (rank === 2) {
      return '#1A2020'; // 약간 어두운 톤
    }
    if (rank === 3) {
      return '#171D1D'; // 더 어두운 톤
    }
    return '#141A1A'; // 기본 차콜색
  };
  ...
  const getRowBorderStyle = (rank: number) => {
    if (rank === 1) {
      return { 
        borderLeft: '3px solid rgba(57, 229, 194, 0.5)',
        borderRight: '1px solid rgba(57, 229, 194, 0.2)',
        boxShadow: 'inset 0 0 20px rgba(57, 229, 194, 0.1)'
      };
    }
    return {};
  };
  ...
  const getRowClassName = (rank: number) => {
    if (rank === 1) {
      return "hover:border-l-2 hover:border-[#39E5C2] hover:border-opacity-40 transition-all duration-200 animate-pulse-slow";
    }
    return "hover:border-l-2 hover:border-[#39E5C2] hover:border-opacity-40 transition-all duration-200";
  };
          ...
          <Alert variant="destructive" className="rounded-md" style={{ backgroundColor: '#1C2222', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
            ...
            <AlertDescription className="text-gray-300">
              ...
              <Button
                ...
                size="sm"
                onClick={() => refetch()}
                className="mt-3 text-gray-300 hover:bg-[#141A1A]"
                style={{ borderColor: 'rgba(239, 68, 68, 0.3)', backgroundColor: 'transparent' }}
              >
                다시 시도
              </Button>
            </AlertDescription>
      ...
      <div className={`flex-1 overflow-hidden flex justify-center ${isMobile ? 'px-0' : 'px-8'}`}>
        ...
        <div 
          ...
          style={{ 
            ...
            boxShadow: '0 0 20px rgba(57, 229, 194, 0.15), 0 0 40px rgba(57, 229, 194, 0.08)'
          }}
        >
          {/* 상단 민트 하이라이트 라인 */}
                ...
                {youngRanking.map((item, index) => (
                  <TableRow 
                    ...
                    style={{ 
                      ...
                      borderBottom: '0.6px solid rgba(57, 229, 194, 0.1)',
